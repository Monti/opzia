...
async function crazyDeploy(tokenName, tokenSymbol) {
  const factory = await UniswapFactory.deployed();

  const optionFactory = await OptionRegistryFactory.deployed();

  const mockToken = await MockToken.new(tokenName, tokenSymbol);
  ...
  return web3.eth.getAccounts().then(async accounts => {
    ...
    if (exchangeSupply.toString() != 10 * 10 ** 18) {
      ...
      await mockToken.approve(tokenExchange.address, web3.utils.toWei("1000"), {
        from: accounts[0]
      });
    ...
    let offer = null;
    if ((await mockTokenRegistry.getUserOffersLength.call(accounts[0])) == 0) {
    ...
    let tokenOffer = null;
    if ((await mockTokenRegistry.getUserOffersLength.call(accounts[0])) == 1) {
      ...
      await mockToken.approve(mockTokenRegistry.address, maxAssets, {
        from: accounts[0]
      });
...
const NAME_SYMBOL = [["Mock", "MCK"], ["EOS", "EOS"], ["Ethereum", "ETH"]];

module.exports = async function(callback) {
  console.log("STARTING");
  let addressPromises = NAME_SYMBOL.map(ite => crazyDeploy(ite[0], ite[1]));
